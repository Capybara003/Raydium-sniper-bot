...
import { RayLiqPoolv4, connection, LP_wallet_keypair } from '../../config';
import { IPoolKeys } from './interface';
...
export async function derivePoolKeys(marketId: PublicKey) {
  ...
  const ownerBaseAta = await getOwnerAta(baseMint, LP_wallet_keypair.publicKey);
  ...
  const ownerQuoteAta = await getOwnerAta(quoteMint, LP_wallet_keypair.publicKey);
