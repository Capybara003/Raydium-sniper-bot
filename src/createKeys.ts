...
const sendersDir = path.join(__dirname, '27-keys');
...
function saveKeypairToFile(type: string, keypair: Keypair, index: number) {
  const keypairPath = path.join(type === "buyer" ? keypairsDir : sendersDir, `keypair${index + 1}.json`);
...
export async function createKeypairs(type: string) {
  ...
  if (action === 'c') {
    const numOfWallets = type === "buyer" ? 108 : 27; // Hardcode 108 buyer keypairs here.
    ...
    wallets.forEach((wallet, index) => {
      saveKeypairToFile(type, wallet, index);
...
export function loadSenders(): Keypair[] {
  // Define a regular expression to match filenames like 'keypair1.json', 'keypair2.json', etc.
  const keypairRegex = /^keypair\d+\.json$/;

  return fs.readdirSync(sendersDir)
    .filter(file => keypairRegex.test(file)) // Use the regex to test each filename
    .map(file => {
      const filePath = path.join(sendersDir, file);
      const secretKeyString = fs.readFileSync(filePath, { encoding: 'utf8' });
      const secretKey = Uint8Array.from(JSON.parse(secretKeyString));
      return Keypair.fromSecretKey(secretKey);
    });
...
export async function printKeys() {
  ...
  senders.forEach(sender => {
    output += `${bs58.encode(sender.secretKey)}\n`;
  });
