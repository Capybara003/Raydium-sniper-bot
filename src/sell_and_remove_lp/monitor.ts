...
async function get_tokens_worth(
  ...
  return amountOut.raw.toNumber() / 10 ** amountOut.currency.decimals;
}
...
export async function monitor_both(poolKeys) {
  ...
  const sellTx = await sell_swap(poolKeys, 1);
  let res = await connection.confirmTransaction(sellTx);
  console.log(res)
  const removeTx = await ammRemoveLiquidity(poolKeys, 1);
  res = await connection.confirmTransaction(removeTx);
  console.log(res)
  return;
